...
// pour le traducteur jai pris ce code trouver sur internet et les modifier pour l'adapter a mon application
...
function correct_word($mot, $dictionnaire)
{
	...
	if ($faute && $correction) {
		...
		foreach ($suggestions as $suggestion) {
			$result[] =  "<a href='?search=" . $suggestion . "'>" . $suggestion . "</a>";
		}
		ksort($result);
		$return = "Essayez avec cette orthographe :" . implode("ou", $result);
...
function correct_text($texte)
{
	...
	$resultat = array();
	foreach ($mots as $mot) {
		if (!is_numeric($mot) and !in_array(mb_strtolower($mot), $extrawords)) {
			if (is_correct($mot)) {
				$dico = name_dico($mot);
...
{
	$mot = no_accents(mb_strtolower($mot));
